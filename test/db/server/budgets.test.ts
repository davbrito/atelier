...
describe("createBudget", () => {
  it("slugifies the name and inserts materials/operations", async () => {
  ...
  it("dedupes the slug when another budget already has it", async () => {
    const org = await seedOrganization(db);
...
describe("updateBudget", () => {
  it("replaces materials on update", async () => {
    const org = await seedOrganization(db);
  ...
  it("keeps its own slug when the name doesn't change", async () => {
    const org = await seedOrganization(db);
    const budget = await db.transaction((tx) =>
      createBudget(tx, org.id, {
        name: "Vestido",
